...
**Answer: B) How to increase transaction capacity while maintaining decentralization**

The war wasn't about Bitcoin's basic mechanics—it was about how to scale the network. Big blockers wanted larger blocks for immediate capacity gains, while small blockers worried this would make running nodes too expensive, centralizing the network among wealthy corporations.
...
**Answer: B) Increase the block size limit to fit more transactions**

Big blockers believed the simplest solution was to expand Bitcoin's "clubs" (blocks) to hold more "customers" (transactions). They proposed increasing from 1MB to 2MB, 8MB, or even unlimited block sizes to eliminate congestion and high fees.
...
**Answer: C) Only wealthy corporations could afford to run nodes**

Small blockers feared that larger blocks would require more storage, bandwidth, and processing power. This would price out regular people from running their own NodeBuddy operations, potentially centralizing Bitcoin among big corporations—the exact opposite of Bitcoin's decentralized vision.
...
**Answer: B) It moved signature data to a separate area**

SegWit (Segregated Witness) was like adding a "coat check" to Club Mempool. By separating transaction signatures from the main transaction data, more transactions could fit in the same 1MB space—effectively increasing capacity by about 70% without changing the base block size limit.
...
A) Bitcoin was shut down permanently
...
**Answer: C) Bitcoin split into Bitcoin (BTC) and Bitcoin Cash (BCH)**

Unable to reach consensus, the Bitcoin community literally split in two. Bitcoin (BTC) continued with SegWit and the small blocker roadmap, while Bitcoin Cash (BCH) forked off with 8MB blocks and the big blocker vision. Both claimed to be the "real" Bitcoin, but BTC retained majority support.
...
**Answer: B) Bitcoin (BTC) maintained dominance while BCH found a niche**

Bitcoin (BTC) kept its position as the #1 cryptocurrency, with SegWit and Lightning Network enabling scaling while preserving decentralization. Bitcoin Cash found success as a lower-fee alternative but never challenged BTC's dominance, maintaining roughly 1-2% of BTC's market cap.
